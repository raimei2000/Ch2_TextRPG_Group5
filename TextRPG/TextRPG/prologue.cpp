#include "prologue.h"
#include "page_utils.h"

#include <iostream>
#include <string>
#include <cstdlib>
#include <vector>

void PrintPrologue(const std::string& player_name) {
  std::vector<std::string> epilogue_lines = {
  "",
  "========================================",
  "          제13 던전 입장 안내",
  "========================================",
  "",
  "당신, " + player_name + "은(는) 오늘 처음 모험가 등록을 마친",
  "초보 모험가입니다.",
  "당신은 의뢰 게시판에서",
  "눈에 띄는 게시판을 보게 됩니다."
  };

  EndingScroll(epilogue_lines);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }

  std::vector<std::string> epilogue_lines2 = {

   "             +({xi ",
   "             >?[)/}                                       ,+]/t|1!  ",
   "             i-]{(]                                       l!? / fj]<                         ",
   "        >[1tf|1tffjvccrnxtnucUr|J0QQUCzYXvufvXXnvUCQJQJXurczftzxcnxzXn]'                    ",
   "        :<>+]>;<i+}{-|rrjjr/ftrux))|rxnurjunvucn(|unjj|}(j[i->|tfggsyft{+                    ",
   "       ;;;;l``.`^;!'':::II; ,',:Illl; l!i!iIl; I!i!!!i!;; II;; I;;II; ^:;l'                    ",
   "            ,<>|                                                {{>l'",
   "            I]1[   ========================================     t(+'l                      ",
   "            >-1]      ✨♚♚제13 던전 초보자 환영 파티♚♚✨        xz1",
   "            l?(]      가입즉시$$신규모험가☜☜낡은검100% 증정     1//?  ",
   "            >}t(       ♜경력무관♜레벨무관♜생존경험무관￥        cc/>",
   "            <}[?  §§몬스터처치§§골드획득★★희귀아이템30 % 기회@@ tfx~",
   "            +1]+      믿음직한동료♬안전한모험♬화려한보상♬       rt)1",
   "            :~((     보스만잡으면즉시퇴장가능☞☞지금바로참가!!!  /t[",
   "            i-([   ========================================     |_l",
   "            i}/?                                                (/)}  ",
   "            l[|]{nnLvYY0JcLpkkbdqpbkdpdddbbdqkhoppZpwpZmwOLCjfxdwjj( ",
   "            `l_-1_v}|fuX00zCLZmwYZmqdqqdbkkkbahk0UpqZXcYXfzcreds})/}",
   "            l]]]u},[/)uxxuvr{jfcLvLCQO0ZQJZpJfzzYC0CLzcJUu!zfadxf1){_",
   "           l!<<+!il_]i][}}-1(jtft(1]+__1)(/|()ttt|rjx|[?1-1>-ffffff[i ",
   "             `'^^;:,^ ;'';,' ^ '      .^.  ';^   ,^`''^I;``^''   ",
   "              :l~[/?i :,                                ,l+~|+i^  ",
   "              !<_]|/-!                                  ll?)j}< ",
   "              l<+}|]_i                                  !i/fx(+ ",
   "              l_<{f(1!                                  i?tfj/]   ",
   "              l+?{r/{I                                  '<-? (rx",
   "              ;!+((j]!                                  `!!{{[|[+ ",
   "              +I+}|1{!                                  I+(|}f~   ",
   "              ;!<-}-1!                                  ;+{}t1_  ",
   "              :i+}[{}<                                   -_{<)_",
   "               I<1-                                       +]     "



  };

  ShowAllAtOnce(epilogue_lines2);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }

  std::vector<std::string> epilogue_lines3 = {
  "i-([     ========================================================   |_l",
  "i}/?         ※동료의 생존 여부는 당사가 책임지지 않습니다.         (/)}  ",
  "l[|]     ※아이템 획득 확률은 모험가의 체감과 다를 수 있습니다.     dwjj( ",
  ",<>|                                                               {{>l'",
  "`l_-1_v}|fuX00zCLZmwYZaweWASDFmqdqqdbkdddkkckkkkbahk0UpqZXcYXfzcreds})/}",
  "l]]]u},[/)uxxuvr{jfcLvLCQO0ZQJZDDSSSFFpJfzzzzzewwqYC0CLzscJUu!zfadxf1){_",
  "",
  player_name + "은(는) 화려하게 번쩍이는 문구에 정신이 팔린 나머지,",
  "아래에 작게 적힌 주의사항을 읽지 못했습니다.",
  "그리고 등록한 지 10분 만에,",
  "보스 토벌 파티의 마지막 자리에",
  "강제로 배정되었습니다."
  };

  EndingScroll(epilogue_lines3);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }
  std::vector<std::string> epilogue_lines4 = {
  "\"✨♚♚제13 던전 초보자 환영 파티♚♚✨\"라는 설명과 달리",
  "당신을 기다리고 있던 것은 음산한 던전과",
  "조금도 환영할 생각이 없어 보이는",
  "몬스터들이었습니다.",
  "",
  "던전에 들어선 지 얼마 지나지 않아",
  "정체불명의 몬스터들이 파티를 습격했습니다.",
  "",
  "비명과 고함이 뒤섞였고,",
  "당신은 강한 충격과 함께 정신을 잃었습니다.",
  "",
  "정신을 차렸을 때,",
  "당신과 함께 싸울 수 있는 동료는",
  "아무도 남아 있지 않았습니다."
  };

  EndingScroll(epilogue_lines4);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }

  std::vector<std::string> epilogue_lines5 = {
  "전투의 흔적이 가득한 방 한가운데에서는",
  "술 취한 오크 한 마리가 동료의 투구에",
  "술을 따라 마시며 혼자 낄낄거리고 있었습니다.",
  "",
  "\"인간들 장비는 약해도",
  "술잔으로는 쓸 만하군!\""
  };

  EndingScroll(epilogue_lines5);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }

  std::vector<std::string> epilogue_lines6 = {
  "방구석에는 거대한 트롤이 웅크리고 앉아",
  "무언가를 정신없이 씹고 있었습니다.",
  "",
  "트롤은 당신과 눈이 마주치자",
  "움직임을 멈추고 낮게 중얼거렸습니다.",
  "",
  "\"단백질...... 더 필요해......\""
  };

  EndingScroll(epilogue_lines6);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }

  std::vector<std::string> epilogue_lines7 = {
  "그 옆에서는 악취 고블린들이",
  "쓰러진 동료들의 가방을 뒤지고 있었습니다.",
  "",
  "\"반짝이는 건 내 거다!\"",
  "\"먹을 수 있는 것도 내 거다!\"",
  "\"냄새나는 건...... 난 아니야!!\"",
  "",
  "당신은 숨을 죽인 채",
  "천천히 낡은 검을 움켜쥐었습니다.",
  "",
  "두려움 때문인지,",
  "지독한 고블린 냄새 때문인지는 알 수 없었지만",
  "눈에서는 계속 눈물이 흘렀습니다."
  };

  EndingScroll(epilogue_lines7);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }


  std::vector<std::string> epilogue_lines8 = {
  "그 순간,",
  "던전 전체에 상황과 전혀 어울리지 않는",
  "밝고 경쾌한 목소리가 울려 퍼졌습니다.",
  "",
  "[던전 관리 시스템]",
  "",
  "\"파티원 전원의 전투 불능을 확인했습니다.\"",
  "",
  "\"생존자 1명을 임시 파티장으로 승격합니다.\"",
  "",
  "\"축하합니다, " + player_name + " 님!\""
  };

  EndingScroll(epilogue_lines8);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }

  std::vector<std::string> epilogue_lines9 = {
  "축하할 상황은 전혀 아니었습니다.",
  "",
  "다행히 던전 관리 시스템은",
  "각 구역 사이에 안전지대를 운영하고 있었습니다.",
  "",
  "안전지대에서는 현재 상태를 확인하거나,",
  "몬스터에게서 획득한 골드로",
  "무인 상점을 이용할 수 있습니다.",
  "",
  "하지만 당신이 들어온 던전의 문은",
  "어느새 흔적도 없이 사라져 있었습니다."
  };

  EndingScroll(epilogue_lines9);

  if ( EnterToNextPage("프롤로그") ) {
    return;
  }

  std::vector<std::string> epilogue_lines10 = {
  "[던전 관리 시스템]",
  "",
  "\"퇴장을 원하신다면",
  "최하층의 보스를 처치하세요.\"",
  "",
  "\"참고로 중도 포기와 환불은 불가능합니다.\"",
  "",
  "당신은 낡은 무기를 고쳐 잡았습니다.",
  "",
  "살아남기 위해서,",
  "사라진 동료들의 복수를 위해서,",
  "그리고 이 엉망인 던전에서 퇴근하기 위해서.",
  "",
  "초보 모험가 " + player_name + "의",
  "예정에 없던 단독 공략이 지금 시작됩니다.",
  "",
  "========================================",
  "",
  "Enter 키를 눌러 게임을 진행합니다.."
  };

  EndingScroll(epilogue_lines10);

  ShowCursor();
  std::string input;
  std::getline(std::cin, input);

  std::cout << std::endl;
  std::cout << std::endl;
}